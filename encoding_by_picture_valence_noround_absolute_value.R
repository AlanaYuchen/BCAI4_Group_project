...
mean_valence
mean_valence$absolute_valence = abs(mean_valence$Valence-2)
mean_valence
...
library(ggplot2)
ggplot(data=combine_encoding_memorability_with_valence,mapping=aes(x=absolute_valence,y=encoding_memorability))+
  geom_smooth()+
  theme_classic()+
  geom_point() + 
  ...
  labs(x="Absolute Valence", y="Memorability (%)") +
...
model = lm (encoding_memorability~absolute_valence, data = combine_encoding_memorability_with_valence)
summary(model)
library(ggplot2)
ggplot(data=combine_encoding_memorability_with_valence,mapping=aes(x=absolute_valence,y=encoding_memorability))+
  geom_abline(intercept = 0.25045, slope= 0.27662, size = 1.3)+
  theme_classic()+
  geom_point() + 
  theme(axis.line.x=element_line(linetype=1,color="black",size=1.2))+
  theme(axis.line.y=element_line(linetype = 1, color = "black", size = 1.2))+
