...
mean_valence$rounded = round(mean_valence$Valence)
mean_valence$calibrated = 4-mean_valence$rounded
mean_valence
...
combine_encoding_memorability_with_valence$rounded = as.factor(combine_encoding_memorability_with_valence$rounded)
combine_encoding_memorability_with_valence$calibrated = as.factor(combine_encoding_memorability_with_valence$calibrated)

### aov of encoding_memorability against Valence. 
model = aov(encoding_memorability~calibrated, data = combine_encoding_memorability_with_valence)
summary(model)
TukeyHSD(model)

### plotting the encoding_memorability against valence.
library(ggplot2)
plot1 = ggplot(combine_encoding_memorability_with_valence, aes(x = calibrated, y = encoding_memorability)) 
...
nums = c(23, 28, 21)
Type = c("Positive", "Neutral", "Negative")
data = data.frame(type=type, nums=nums)
p <- ggplot(data = data, mapping = aes(x = "Valence", y = nums, fill = Type)) + 
...
### statistics
positive = combine_encoding_memorability_with_valence[which(combine_encoding_memorability_with_valence$rounded==1)]
positive
